...
public class AVLTree<E extends Comparable<? super E>> {
    ...
    public E deleteMin() {
        ...
        E minVal = findMin(root);
        root = reDeleteMin(root);
        return minVal;
    }
    private AvlNode reDeleteMin(AvlNode node) {
        if (node == null) {
            return null;
        }
        if (node.left!=null) {
            node.left=reDeleteMin(node.left);
        }
        else {
            node = node.right;
        }
    ...
    private E findMin(AvlNode node) {
        if (node.left == null) {
            return node.value;
        }
    ...
    private void rePrintTree(AvlNode node, int depth) {
        if (node == null) {
            return;
        }
        String indentStr = "";
        ...
        if (node.value != null) {
            ...
            System.out.println(indentStr + node.value + "(" + node.height + ")");
